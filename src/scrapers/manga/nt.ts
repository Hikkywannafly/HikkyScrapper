...
import { proxyServer } from '../../lib/serverProxy';
export default class NettruyenScraper extends MangaScraper {
  constructor() {
    super('nt', 'NT', { baseURL: 'https://nettruyenco.vn' });
  ...
  async getImages(query: GetImagesQuery) {
    ...
    const { data } = await this.client.get(
      `/truyen-tranh/${source_media_id.split('-').slice(0, -1).join('-')}/chapter-0/${chapter_id}`,
    );
  ...
  composeImages(html: string) {
    ...
    return images.toArray().map((el) => {
      ...
      const imageCheckProtocols = protocols.some((protocol) => source.includes(protocol))
        ? source
        : `https:${source}`;
      const image = this.handleProxyImage(imageCheckProtocols);
      return {
        image,
      };
    });
  ...
  handleProxyImage(image: string) {
    const ImageWithProxy = `${proxyServer}/proxy?url=${this.baseURL}&src=${image}`;

    return ImageWithProxy;
  }
